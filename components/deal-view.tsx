                  ...
                  return (
                    <TableRow
                      ...
                      className={`group hover:bg-muted/20 transition-colors ${
                        isLastInGroup && groupIdx < groups.length - 1
                          ? "border-b-2 border-border"
                          : ""
                      }`}
                    >
                      {/* Solution Type — only render cell on first row, span rest */}
                      {isFirstInGroup && (
                        <TableCell
                          className="pl-6 py-4 align-top font-medium text-sm text-foreground"
                      ...
                      <TableCell className="py-4">
                        <span className="text-sm font-medium tabular-nums text-foreground">
                          {item.deal.dealYear}
                          {item.deal.dealYearEnd ? `–${item.deal.dealYearEnd}` : ""}
                        </span>
                      </TableCell>
                      ...
                      <TableCell className="py-4">
                        <div className="flex items-center gap-2.5">
                          {item.deal.partnerUrl && (
                            <PartnerLogo domain={item.deal.partnerUrl} />
                          )}
                          <span className="text-sm font-medium text-foreground">
                            {item.deal.partnerName}
                          </span>
                        </div>
                      </TableCell>
                      ...
                      {/* Deal Type Checkmark Columns */}
                      {DEAL_TYPE_COLUMNS.map((col) => (
                        <TableCell key={col} className="text-center py-4">
                          {item.deal.dealType === col && (
                            <CheckCircle2 className="w-5 h-5 text-brand-blue/70 mx-auto" />
                          )}
                        </TableCell>
                      ))}
                      ...
                      <TableCell className="text-center pr-6 py-4">
                        <Button
                          ...
                          aria-label={`View details for ${item.deal.partnerName} deal`}
                        >
